...
```*_NUM_THREADS``` variables and [```n_jobs```](publication/massbank/run_with_gridsearch.py#L83) setting with respect to the actual number of your **physical** cores, i.e. ```*_NUM_THREADS``` + ```n_jobs``` = number of physical cores available.
